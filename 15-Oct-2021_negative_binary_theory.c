...
Если возьмем двоичное число 1111 1111 и прибавим к нему еще 1 (т.е. 0000 0001 в двоичном виде), то получится 1 0000 0000,
которое в тип char уже не влезет, произойдет переполнение и, в итоге, в ячейке памяти окажется 0000 0000, то есть 0.

А если при прибавлении 1 к 1111 1111 получается 0, то можно считать 1111 1111 представлением числа -1.

"По определению, −1 — это такое число, при прибавлении к которому единицы (нейтрального элемента относительно операции умножения)
получается ноль (нейтральный элемент относительно операции сложения)." (https://ru.wikipedia.org/wiki/%E2%88%921_(%D1%87%D0%B8%D1%81%D0%BB%D0%BE))

Этот принцип и взят за основу представления отрицательных чисел в современных компьютерах.
Такое представление еще называется "дополнительным кодом"
...
int main()
{
	...
	c = -1; // было (как бы отрицательное) 1111 1111 -> 1111 1111, сдвинули на 1 влево и на освободившееся место опять поставили 1
		// (т.к., число должно остаться отрицательным)
